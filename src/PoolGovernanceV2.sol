...
contract PoolGovernanceV2 is Ownable {
    uint public constant epochInterval = 1 days;
    uint public constant votingRatio = 66; // % of agreements required
    ...
    function proposeEpoch(Epoch memory epoch) external onlyOperator {
    ...
    function deleteOperators(address[] memory _operators) external onlyOwner {
        for (uint i = 0; i < _operators.length; i++) {
    ...
    ) private view returns (Epoch[] memory _votes) {
    ...
    function _isVoteEqual(
        ...
        if (vote1.state == 0 && vote1.withdrawals == 0 && vote1.exits == 0) {
            return false;
        } else if (
            keccak256(abi.encode(vote1)) == keccak256(abi.encode(vote2))
        ) {
            return true;
        }
        return false;

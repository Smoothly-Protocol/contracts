...
contract PoolGovernance is Ownable {
  uint32 constant internal epochInterval = 1 days;
  uint8 constant internal votingRatio = 66; // % of agreements required
  uint64 public epochNumber = 0;
  ...
  function proposeEpoch(Epoch calldata epoch) onlyOperator external {
  ...
  function addOperators(address[] calldata _operators) onlyOwner external {
    for (uint256 i = 0; i < _operators.length; i++){
  ...
  function deleteOperators(address[] calldata _operators) onlyOwner external {
    for (uint256 i = 0; i < _operators.length; i++){
  ...
  function _isVoteEqual(Epoch memory vote1, Epoch calldata vote2) private pure returns(bool) {
  ...
  function _distributeRewards(uint256 fee, address[] memory _operators) private {

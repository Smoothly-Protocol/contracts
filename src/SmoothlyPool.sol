...
contract SmoothlyPool is Ownable, ReentrancyGuard {
  ...
  uint constant SLASH_MISS = 0.015 ether;
  uint constant SLASH_FEE = 0.05 ether;
  ...
  function addStake(uint id) external payable {
    Validator storage v = validators[msg.sender][id];
    require(v.stake > (STAKE_FEE - SLASH_FEE), "Validator not allowed to add more stake");
    require((msg.value + v.stake) <= STAKE_FEE , "Stake fee bigger than allowed");
    v.stake += msg.value;
